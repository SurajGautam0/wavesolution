...
export function WhatsAppWidget() {
    ...
    return (
        <div className={cn(
            "fixed bottom-6 right-6 z-50 flex flex-col items-end gap-2 transition-all duration-500 transform",
            isVisible ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"
        )}>
            {/* Chat Bubble Popup */}
            <div className={cn(
                "bg-white rounded-xl shadow-xl p-4 mb-2 max-w-[250px] transition-all duration-300 origin-bottom-right border border-slate-100 relative",
                showPopup ? "scale-100 opacity-100" : "scale-0 opacity-0 pointer-events-none"
            )}>
                <button
                    onClick={(e) => {
                        e.preventDefault();
                ...
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-500">Need help with cleaning?</p>
                    </div>
                </div>
                <Link
            ...
            >
                <span className="absolute w-full h-full rounded-full bg-[#25D366] opacity-75 animate-ping group-hover:opacity-0"></span>
